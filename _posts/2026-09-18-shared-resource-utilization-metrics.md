...
| 그럼 왜 못 쓰나 | 낭비 **1,742시간** — 자투리 42% · 미반납 · 지각 · 노쇼 |
...
| 갖춘 것 | 볼 수 있는 것 |
|---|---|
| **예약 시스템만** | 점유율 · 회전율 · 희망 시간 확보율 · 대기 · 자투리 · **유도 가능 대기율** |
...
**연속된 두 예약 사이의 틈이 한 회 길이보다 짧아 아무도 못 쓰는 시간.**

```
예약 A 09:00~10:00 │ 틈 30분 │ 예약 B 10:30~11:30
                     └─ 한 회가 60분이면 이 30분은 아무도 못 쓴다
```

- 틈이 한 회 길이 **이상**이면 자투리가 아니다(누군가 쓸 수 있다)
- 한 회 길이가 없는 자원(자유 시간 예약)은 **"못 쓰는 틈"을 정의할 수 없어 세지 않는다**

이 값이 낭비 1위로 올라오면 처방이 알림이 아니라 **예약 시작 시각 정렬**이다.
...
| 지표 | 계산 |
|---|---|
| **회수 가능 회차** | `(late + idle + noshow + fragment) ÷ 한 회 길이 ÷ 일수` |
...
여기에 한 가지 판단이 더 붙는다. 지각·미반납·노쇼(**행동**)와 자투리(**배치**)는
고치는 주체가 다르다.

| 조건 | 결론 |
|---|---|
| 행동 3종이 낭비의 50% 이상 | 알림·정책으로 푼다. 싸다 |
| 자투리가 더 큼 | 예약 시간 설계를 손봐야 한다. 알림을 아무리 보내도 안 줄어든다 |
...
재슬롯팅했을 때 회전율·대기·자투리가 어떻게 변하는지 계산한다.
...
- **낭비를 앞·뒤·부재·자투리로 쪼갠다.** 처방이 다르기 때문이다. 합쳐서 한 숫자로 두면
  "크다"까지만 알고 뭘 할지는 모른다.
