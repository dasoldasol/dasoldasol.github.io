...
| 모델 파일 | 학습 입력 | 사용 엔드포인트 |
|-----------|----------|----------------|
| `batch_models.pkl` | title + request_contents + reply | `/classify-with-reply`, 배치 AI 오버레이 |
| `api_models.pkl` | title + request_contents (reply 제외) | `/classify` |
...
**작업유형 분류:** AI 단독 사용 (키워드 대비 정확도 우세)
...
기본값은 400이며, 배치 학습 시 CV(교차 검증) 기반으로 최적 alpha를 자동 탐색하여 `alpha_config.json`에 저장한다. API는 이 값을 로드하여 사용한다.

| alpha | 계산 방식 | 적용 |
|-------|----------|------|
| 400 (기본값) | 배치 학습 시 CV 기반 자동 최적화 | 배치 + API |
...
case "$1" in
    ...
    api)       exec uvicorn api.main:app --host 0.0.0.0 --port 80 "$@" ;;
esac
```
